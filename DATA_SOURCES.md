...
### E3. USGS ★ free and deep — confirmed working, including the historical products
- **Data Series 140** — **confirmed**: one static file, `ds140-gold-2022.xlsx`, US primary/secondary production, imports, exports, consumption, unit value, and world production total, **annual 1900–2022** in one clean table. `src/pull_usgs_historical.py` → `data/processed/usgs_ds140_gold_annual.csv` (123 rows).
- **Minerals Yearbook, Table 8 (World Mine Production by Country)** — **confirmed**: one file per publication year (~2006–2022 found; each file carries a 5-year rolling window, e.g. the 2022 edition covers 2018–2022), giving real country-level annual mine production back to 2002 once combined. `src/pull_usgs_historical.py` → `data/processed/usgs_myb_world_production_by_country.csv` (2,121 rows, 135 countries). Confirms the expected pattern for this project's five countries: China ~330–370 t/yr, USA ~173–193 t/yr, India ~1.2 t/yr, UK ~28–50 **kg**/yr (negligible — not a mining country), Switzerland absent from the table entirely (no domestic production).
- **Mineral Commodity Summaries (MCS)** — annual headline; confirmed live PDF archive back to 1996 at predictable URLs (`pubs.usgs.gov/periodicals/mcs{year}/mcs{year}-gold.pdf`). **Parsed** (`src/extend_usgs_production_series.py`, via `pypdf` text extraction): the world-mine-production table names the ~13 largest producers each edition (varies slightly year to year) plus an "Other countries" catch-all, with far less lag than the Minerals Yearbook — the Feb-2026-dated edition already has 2025-estimated figures. Used to extend the Minerals Yearbook's country series from 2022 to ~2025 for whichever countries MCS names — **China and USA every edition checked; India, UK and Switzerland never (too small, folded into "Other countries")**. Each edition covers two years (prior year final/revised + current year estimated); later editions' figures for a shared year are preferred. Cross-checked the extended US figures against MIS's actual monthly production (already pulled) — matched almost exactly (e.g. 2024: 163.0 t both ways).
...
- **Access:** `usgs.gov/centers/national-minerals-information-center/gold-statistics-and-information` needs a browser User-Agent (403 without one, same pattern as BAZG's CloudFront check) — but the actual files sit on an unprotected public S3 bucket. Two path conventions found across years: `d9-wret.s3.us-west-2.amazonaws.com/assets/palladium/production/s3fs-public/media/files/mis-{YYYYMM}-gold.xlsx` (monthly, and DS140) and `.../mineral-pubs/gold/myb1-{YYYY}-gold.xls` for older Minerals Yearbook editions, switching to `pubs.usgs.gov/myb/vol1/{YYYY}/myb1-{YYYY}-gold.xlsx` from 2018 on. ("palladium" in the path is just USGS's S3 bucket naming, not a commodity filter — same path serves gold fine.)
- **Gotchas:** MIS xlsx confirmed live back to 2022-01 at this path (each month's file also restates the full prior calendar year, so pulling from 2022-01 onward gives 2021+ coverage); 2021 and earlier 403 at this exact path, not investigated further. USGS's per-country import/export breakdown in MIS is only published for the most-recent month in each file — the monthly pull script extracts totals only, not the full category/country detail. Minerals Yearbook country names carry footnote-digit suffixes (e.g. "Burkina Faso3") — stripped by the pull script, worth double-checking on edge cases.
