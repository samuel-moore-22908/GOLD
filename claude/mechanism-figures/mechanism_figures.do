...
*==============================================================================
* Estimated on January 2023 to the end of the data. 2020 and the March 2022
...
* puts it at $3.77, well inside its own interval.
...
local GELO = g_e_lo[1]
local GEHI = g_e_hi[1]
local BBLE = b_below_e[1]
local R2E  = r2_e[1]
local R2LE = r2_lin_e[1]
local CORE = corr_e[1]
di as txt "west: g=" as res %5.2f `G' as txt " CI [" as res %5.2f `GLO' ///
    as txt ", " as res %5.2f `GHI' as txt "]   east: g=" as res %5.2f `GE' ///
    as txt " slope below " as res %5.2f `BBLE'

tempfile hingeE
import delimited using "$PROJ/kink_fit_east.csv", varnames(1) clear
rename yhat yhatE
gen byte ishinge = 1
save `hingeE'
...
gen byte ishinge = 1
merge 1:1 x using `hingeE', nogen
save `hinge'
...
keep if on_scatter == 1 & ym >= "2023-01"
...
twoway ///
  ...
  text(186 `=`G'+0.7' ///
      "estimated threshold, {c $|}`=string(`G',"%3.2f")' an ounce" ///
      "shaded: 90% interval, {c $|}`=string(`GLO',"%3.2f")' to {c $|}`=string(`GHI',"%4.2f")'", ///
      place(e) size(vsmall) color("`INK'%80") justification(left)) ///
  text(74 20.5 "above it, `=string(`BABV',"%3.1f")' tonnes" ///
      "a month per extra dollar", ///
...
* Same axes, same scale, same threshold strip, and now a fitted line of its own.
*
* Extending the window to the present is what made drawing it defensible. On the
* shorter window the below-kink slope rested on two discount months; it now
* rests on nine, and the earlier flat claim - that this leg shows no price
* response at all - was too strong. What survives is weaker and the panel says
* so in figures rather than in adjectives: the threshold interval spans zero,
* the straight-line R-squared is 0.01, and the correlation is -0.11 on 43
* months. The line is drawn; the reader is told not to lean on it.
twoway ///
  `BAND' ///
  ...
  (line yhatE x if ishinge == 1, ///
      lcolor("`INK'%55") lwidth(medthick) lpattern(solid)) ///
  , ///
  `XAX' `YAX' `GLINE' ///
  ytitle("", size(vsmall)) ///
  text(186 -8.6 "fitted the same way, and not to be leaned on:" ///
      "R-squared `=string(`R2E',"%3.2f")', against `=string(`R2LE',"%3.2f")' for a straight line," ///
      "and a threshold interval that spans zero", ///
      place(e) size(vsmall) color("`INK'%75") justification(left)) ///
  `REGION' legend(off) ///
  title("Eastward: New York back to Switzerland", size(small) ///
      color("`INK'") position(11) justification(left)) ///
  subtitle("Not arbitrage. A slope only where New York is at a discount", ///
      size(vsmall) color("`SOFT'") position(11) justification(left)) ///
...
graph combine f3a f3b, cols(2) ycommon ///
    ...
    `TAB' ///
    subtitle("{bf:One leg is priced. The other barely is}" ///
        "Swiss customs gold trade with the United States against the New York" ///
        "premium. Each dot is one month, January 2023 to July 2026", `HEAD') ///
    note("Estimated from January 2023 forward: 2020 and the 2022 sanctions shock are" ///
        "location dislocations of a different kind, and folding them in widens the" ///
        "claim past what this paper argues. It costs precision, not the result - the" ///
        "full 2015-2026 series puts the westward threshold at {c $|}2.99 against {c $|}`=string(`G',"%3.2f")' here." ///
        ...
        "2,000 iid and moving-block bootstrap replications. On `=string(`N',"%2.0f")' months that" ///
        "interval runs {c $|}`=string(`GLO',"%3.2f")' to {c $|}`=string(`GHI',"%4.2f")', so the threshold is well located and poorly pinned." ///
        "Westward: R-squared `=string(`R2',"%3.2f")' against `=string(`R2L',"%3.2f")' for a straight line, and a slope of" ///
        "`=string(`BBLW',"%3.1f")' tonnes per dollar below the threshold against `=string(`BABV',"%3.1f")' above it." ///
        "Eastward: R-squared `=string(`R2E',"%3.2f")' against `=string(`R2LE',"%3.2f")', correlation `=string(`CORE',"%4.2f")', and a threshold" ///
        "interval of -{c $|}`=string(-`GELO',"%3.2f")' to +{c $|}`=string(`GEHI',"%3.2f")' that spans zero. Its downward slope where" ///
        "New York is at a discount rests on nine months: report it, do not rely on it." ///
        " " ///
...
*==============================================================================
* Figure 3 is now estimated on everything from January 2023 forward, which is
* exactly the span this chart draws. The shaded strip that used to mark the
* estimation window would therefore cover the whole panel and say nothing, so it
* is gone and the note carries the point instead. (It was also shading the wrong
* thing: Nov 2024 to Nov 2025 is the episode, not the window it was labelled as.)
use `monthly', clear
...
twoway ///
  ...
  text(-95 `=tm(2025m3)' "figure 3 is estimated on this whole span", ///
